...
class WealthsimpleAPI(WealthsimpleAPIBase):
    ...
    def security_id_to_symbol(self, security_id: str) -> str:
        """Convert a security ID to a symbol string.

        If a cache is configured and the security can be looked up, returns
        the symbol in 'EXCHANGE:SYMBOL' format. Otherwise, returns the
        security ID in brackets.

        Note: Some securities (e.g., delisted, private, or special securities)
        may fail to look up via the API, in which case the bracketed ID is returned.
        """
        ...
        if self.security_market_data_cache_getter:
            try:
                ...
                # Some securities cannot be looked up (e.g., delisted, private, or certain ETFs)
                # Return the bracketed security ID as a fallback
                pass
